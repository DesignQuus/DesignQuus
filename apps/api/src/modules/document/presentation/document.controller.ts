import {
  Body, Controller, Get, Headers, Param, ParseFilePipe, Res, StreamableFile,
  Post, UploadedFile, UseInterceptors, MaxFileSizeValidator,
} from '@nestjs/common';
...
export class DocumentController {
  ...
  upload(
    ...
    @Body() body: { projectId: string; projectRevisionId: string; fileType: 'DWG'|'DXF'|'PDF'|'IFC'|'IMAGE' },
    @UploadedFile(new ParseFilePipe({ validators: [new MaxFileSizeValidator({ maxSize: 200 * 1024 * 1024 })] })) file: Express.Multer.File,
  ) {
    return this.documents.upload({ tenantId, ...body, file, idempotencyKey });
  }
  ...
  listExtractions(@TenantId() tenantId: string, @Param('jobId') jobId: string) {
  ...
  promoteSpaces(
    ...
    return this.promotions.promote({
      tenantId, importJobId: jobId, floorId: body.floorId, createdBy: body.createdBy,
    });
  }
  ...
  reviewExtraction(
    ...
    @Body() body: { decision: ReviewDecision; correctedValue?: string; reviewedBy?: string },
  ) {
